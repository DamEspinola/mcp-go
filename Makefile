...
IMG ?= ghcr.io/DamEspinola/mcp-go:placeholder
...
build: fmt vet ## Build CLI binary.
	go build -o bin/mcp-go-$(GO_OS)-$(GO_ARCH) cmd/main.go
...
package: ## Package binary.
	...
	@if [ "$(OS)" = "linux" ]; then \
		tar --transform="s/mcp-go-$(GO_OS)-$(GO_ARCH)/mcp-go/" -cvzf dist/$(PACKAGE_NAME) -C bin mcp-go-$(GO_OS)-$(GO_ARCH) -C ../ LICENSE README.md; \
	elif [ "$(OS)" = "darwin" ]; then \
		tar -cvzf dist/$(PACKAGE_NAME) -s '/mcp-go-$(GO_OS)-$(GO_ARCH)/mcp-go/' -C bin mcp-go-$(GO_OS)-$(GO_ARCH) -C ../ LICENSE README.md; \
	else \
